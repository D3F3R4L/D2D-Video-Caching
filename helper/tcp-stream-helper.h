...
public:
   ...
   */
  TcpStreamServerHelper (uint16_t port, uint32_t simulationId, std::string directory);
  ...
  double GetStorage(Ptr<Node> node);
  void SetStorage(Ptr<Node> node, double value);
...
public:
  ...
  void Handover(Ptr<Node> node, Address ip);
  ...
  void GetBattery(ApplicationContainer clientApps, Ptr<Node> node);
  ...
  double GetEnergy(Ptr<Node> node);
