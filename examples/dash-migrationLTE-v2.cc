...
void LogStall(double sv1, double sv2, double sv3, double cloud)
{
...
void LogRebuffer(double Tsv1, double Tsv2, double Tsv3, double Tcloud)
{
...
void LogStartTime(double sv1, double sv2, double sv3, double cloud)
{
...
void Log(double mah, double choice, int id, std::string alloc)
{
...
*/
void getBatteryClients(ApplicationContainer clientApps, TcpStreamClientHelper clientHelper, std::vector<std::pair<Ptr<Node>, std::string>> clients)
{
...
void getThropughputClients(ApplicationContainer clientApps, TcpStreamClientHelper clientHelper, std::vector<std::pair<Ptr<Node>, std::string>> clients)
{
  for (uint i = 0; i < numberOfUeNodes; i++)
  {
...
void getThropughputServer(ApplicationContainer serverApp, TcpStreamServerHelper serverHelper, NodeContainer servers)
{
...
void ServerHandover(TcpStreamClientHelper clientHelper, Ptr<Node> ue, Address ip)
{
  clientHelper.Handover(ue, ip);
}

void getStartTime(ApplicationContainer clientApps, TcpStreamClientHelper clientHelper, std::vector<std::pair<Ptr<Node>, std::string>> clients)
{
  double STsv1 = 0;
  double STsv2 = 0;
  double STsv3 = 0;
...
void InitializeLogFiles(std::string dashLogDirectory, std::string m_algoName, std::string numberOfUeNodes, std::string simulationId, std::string pol)
{
...
void stopSim(TcpStreamClientHelper clientHelper, NodeContainer staContainer)
{
...
void getStall(ApplicationContainer clientApps, TcpStreamClientHelper clientHelper, std::vector<std::pair<Ptr<Node>, std::string>> clients)
{
  double sv1 = 0;
  double sv2 = 0;
  double sv3 = 0;
  ...
  Simulator::Schedule(Seconds(1), &getStall, clientApps, clientHelper, clients); // precisa verificar a cada 1 segundo, se já verifica a cada 3?
}

void getClientsStallsRebuffers(ApplicationContainer clientApps, TcpStreamClientHelper clientHelper, std::vector<std::pair<Ptr<Node>, std::string>> clients)
{
  for (uint i = 0; i < numberOfUeNodes; i++)
  {
...
void getClientsOnServer(ApplicationContainer serverApp, TcpStreamServerHelper serverHelper, NodeContainer servers)
{
  for (uint j = 0; j < servers.GetN(); j++)
  {
...
void getClientsHandover(ApplicationContainer clientApps, TcpStreamClientHelper clientHelper, std::vector<std::pair<Ptr<Node>, std::string>> clients)
{
  queue[0] = 0;
  queue[1] = 0;
  queue[2] = 0;
...
void politica(ApplicationContainer clientApps, TcpStreamClientHelper clientHelper, std::vector<std::pair<Ptr<Node>, std::string>> clients, ApplicationContainer serverApp, TcpStreamServerHelper serverHelper, NodeContainer servers)
{
...
void politica2(ApplicationContainer clientApps, TcpStreamClientHelper clientHelper, std::vector<std::pair<Ptr<Node>, std::string>> clients, ApplicationContainer serverApp, TcpStreamServerHelper serverHelper, NodeContainer servers)
{
  getClientsOnServer(serverApp, serverHelper, servers);
  getClientsHandover(clientApps, clientHelper, clients);
  getClientsStallsRebuffers(clientApps, clientHelper, clients);
...
void ArrayPositionAllocator(Ptr<ListPositionAllocator> HpnPosition, std::string file)
{
  ...
  {
    NS_LOG_INFO("Adding cell " << cellId << " to coordinates x:" << x_coord << " y:" << y_coord);
...
void choiceServer(int Id, NodeContainer ues, TcpStreamClientHelper clientHelper, TcpStreamServerHelper serverHelper, NodeContainer macroCells, NodeContainer smallCells)
{
...
void storage(int Id, NodeContainer ues, TcpStreamClientHelper clientHelper, TcpStreamServerHelper serverHelper, NodeContainer macroCells, NodeContainer smallCells)
{
  Ptr<Node> client = ues.Get(Id);
  std::vector<double> sizes{90, 146.25, 225, 337.5, 506.25, 765, 1057.5, 1350};
  NodeContainer sv = remoteHosts;
...
void greedy(int Id, NodeContainer ues, TcpStreamClientHelper clientHelper, TcpStreamServerHelper serverHelper, NodeContainer macroCells, NodeContainer smallCells)
{
  Ptr<Node> client = ues.Get(Id);
  std::vector<double> sizes{90, 146.25, 225, 337.5, 506.25, 765, 1057.5, 1350};
  NodeContainer sv = remoteHosts;
...
void aleatorio(int Id, NodeContainer ues, TcpStreamClientHelper clientHelper, TcpStreamServerHelper serverHelper, NodeContainer macroCells, NodeContainer smallCells)
{
  Ptr<Node> client = ues.Get(Id);
  std::vector<double> sizes{90, 146.25, 225, 337.5, 506.25, 765, 1057.5, 1350};
...
void orchestrator(int Id, NodeContainer ues, TcpStreamClientHelper clientHelper, TcpStreamServerHelper serverHelper, NodeContainer macroCells, NodeContainer smallCells)
{
...
{
  std::cout << "At " << Simulator::Now().GetSeconds() << " Cell voltage: " << es->GetSupplyVoltage() << " V Remaining Capacity: " << es->GetRemainingEnergy() / (3.6 * 3.6) << " mAh " << es->GetNode()->GetId() << " node ID" << std::endl;
