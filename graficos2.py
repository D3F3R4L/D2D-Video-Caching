...
parser.add_argument('--segmentfile','-seg',default="segmentSizesBigBuck90.txt", type=str,help='Name of segmentfile used.Default is segmentSizesBigBuck1A.txt')
...
def main():
  print('Beginning')
  segfile='src/esba-dash-energy/dash/{seg}'.format(seg=segmentfile)
  file = open(segfile,"r")
...
def batteryGraphs():
  ...
  discharge=[]
  while i<runs:
    files= '*sim{simu}_*batteryLog*'.format(simu=i)
    batteryFiles = glob.glob(files)
    j=0
    battery=[]
    while j<len(batteryFiles):
      name=batteryFiles[j]
      file=pd.read_csv(name, sep=';',index_col=False)
      mah=np.array(file['Remaining_Capacity_mAh'])
      battery.append(mah[0]-mah[-1]) 
      j+=1
    discharge.append(np.mean(battery))
    i+=1
  ind = np.arange(len(discharge))
  width = 0.4  
  fig, ax = plt.subplots()
  rects1 = ax.bar(ind - width/2, discharge, width,label='Media={d}'.format(d=discharge))
  ax.set_ylabel('used mah')
  ax.set_title('Discharge per simulation')
  ax.set_xticks(ind)
  #ax.set_xticklabels(('D2D', 'Small Cell', 'Macro Cell', 'Cloud'))
  ax.legend()
  save = 'discharge.pdf'
  plt.savefig(save)
  plt.close()
...
def throughputGraphs(numSegments):
  ...
  while(j<len(throughputFiles)):
    ...
    for x1, x2, y1,y2 in zip(Time, Time[1:], Mbps, Mbps[1:]):
      if str(Server[i])=="1.0.0.1":
        ...
        S1Clients[i]=S1Clients[i]+1
      elif str(Server[i])=='2.0.0.1' or str(Server[i])=='1.0.0.3' :
        S2Throughput[i]=S2Throughput[i]+y1
        S2Clients[i]=S2Clients[i]+1
      elif str(Server[i])=='3.0.0.1' or str(Server[i])=='1.0.0.5':
  ...
  plt.xlabel('Seconds')
  plt.ylabel('Mb/s')
  red_line = mlines.Line2D([], [], color='Red',markersize=15, label='D2D')
  green_line = mlines.Line2D([], [], color='g',markersize=15, label='Small Cell')
  blue_line = mlines.Line2D([], [], color='b',markersize=15, label='Macro Cell')
  yellow_line = mlines.Line2D([], [], color='y',markersize=15, label='Cloud')
...
def throughtputServer():
  ...
  plt.xlabel('Seconds')
  plt.ylabel('Mb/s')
  red_line = mlines.Line2D([], [], color='Red',ls='--',lw=3, label='D2D')
  green_line = mlines.Line2D([], [], color='g',ls='-.',lw=3, label='Small Cell')
  blue_line = mlines.Line2D([], [], color='b',ls=':',lw=3, label='Macro Cell')
  yellow_line = mlines.Line2D([], [], color='y',ls='-',lw=2, label='Cloud')
  plt.legend(title='Enforcement Point',handles=[red_line,green_line,blue_line,yellow_line])
...
def qualityGraphs(numSegments):
  ...
  fig,ax =plt.subplots()
  p1,=plt.plot(x,S1QualityMean,color='r',markersize=15, label='D2D')
  p2,=plt.plot(x,S2QualityMean,color='g',markersize=15, label='Small Cell')
  p3,=plt.plot(x,S3QualityMean,color='b',markersize=15, label='Macro Cell')
  p4,=plt.plot(x,S4QualityMean,color='y',markersize=15, label='Cloud')
  plt.xlabel('Chunks')
  ...
  i=0
  fig,ax =plt.subplots()
  p1,=plt.plot([],[],color='r',markersize=15, label='D2D')
  p2,=plt.plot([],[],color='g',markersize=15, label='Small Cell')
  p3,=plt.plot([],[],color='b',markersize=15, label='Macro Cell')
  p4,=plt.plot([],[],color='y',markersize=15, label='Cloud')
  ...
  red_line = mlines.Line2D([], [], color='r',markersize=15, label='D2D')
  green_line = mlines.Line2D([], [], color='g',markersize=15, label='Small Cell')
  blue_line = mlines.Line2D([], [], color='b',markersize=15, label='Macro Cell')
  yellow_line = mlines.Line2D([], [], color='y',markersize=15,label='Cloud')
  width = 0.6
